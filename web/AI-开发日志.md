...
# 2026-09-19 测试问题与安全收敛

- 错题和提交历史由内存分页改为数据库分页。
- V8 迁移增加错题严重程度、查询索引以及用户/源码外键。
- 类似题改为后端模型实时生成，个人中心接通邮箱和密码修改，上传限制统一为 256 KB。
- 未认证响应统一为 401，Token 失效时前端清理登录状态并跳转登录页。
- 安全复查修复 AI 工具跨用户查询风险：模型不再接收用户 ID，工具由服务端认证身份绑定。
- 修改邮箱和密码必须复验当前密码；AI 出题输入与输出增加结构和长度边界。
- 后端回归 97 项，失败 0、错误 0、跳过 3；前端 Vite 生产构建成功。
- 完整问题映射、OCP/代码风格结论、剩余风险和部署检查见 `测试与修复记录.md`。
