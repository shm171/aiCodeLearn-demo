...
- **接口地址**：`PUT /user/{id}`
- **是否需要登录**：是
- **说明**：修改邮箱或密码；敏感修改必须复验当前密码，成功后客户端应重新登录
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| currentPassword | string | 是 | 当前密码，8-16 位 |
| email | string | 否 | 新邮箱；与 password 至少填写一项 |
| password | string | 否 | 新密码，8-16 位；与 email 至少填写一项 |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| file | file | 是 | 源码文件，支持 .java、.cpp、.cc、.cxx，最大 256 KB |
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| category | string | 否 | 按错误分类过滤：FORMAT_ERROR / SYNTAX_ERROR / LOGIC_ERROR |
| severity | string | 否 | 按严重程度过滤：ERROR / WARNING / INFO |
...
- **接口地址**：`POST /core/assistant/practice`
- **是否需要登录**：是
- **说明**：根据错题类型、分类和编程语言实时生成 3 道类似练习

**请求体（JSON）：**

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| errorType | string | 是 | 错误类型或知识点 |
| category | string | 否 | 错误分类 |
| language | string | 否 | 编程语言，默认 C++ |

响应为练习数组，每项包含 `title`、`description` 和 `template`。
